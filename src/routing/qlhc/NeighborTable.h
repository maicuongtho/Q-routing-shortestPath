...
//    void removeOldPositions(simtime_t timestamp);
    std::vector<L3Address> removeOldPositions(simtime_t timestamp);

    void clear();

    simtime_t getOldestPosition() const;

    friend std::ostream& operator<<(std::ostream& o, const NeighborTable& t);
};
